-- Створення бази даних SHOP
CREATE DATABASE SHOP;
GO

-- Використання бази даних SHOP
USE SHOP;
GO

-- Створення таблиці SALES з відповідними типами даних
CREATE TABLE SALES (
    SALES_ORDER_ID BIGINT PRIMARY KEY,          -- Унікальний ідентифікатор замовлення, значення перевищують 2147483647
    REVISION_NUMBER TINYINT,                    -- Інкремент, що не перевищує 255
    ...
    ONLINE_ORDER_FLAG BIT,                      -- Прапорець, де 0 = менеджер, 1 = онлайн
    CUSTOMER_ID INT,                            -- Ідентифікатор клієнта, що не перевищує 2147483647
    SALES_PERSON_ID INT,                        -- Ідентифікатор співробітника, що не перевищує 2147483647
    TERRITORY_ID INT,                           -- Ідентифікатор території, що не перевищує 2147483647
    SUB_TOTAL DECIMAL(18, 4),                   -- Загальна сума замовлення, використання `DECIMAL` для точності
    TAX_AMT DECIMAL(18, 4),                     -- Загальна сума податків
    FREIGHT DECIMAL(18, 4),                     -- Сума доставки
    MODIFIED_DATE DATETIME                      -- Дата і час останньої зміни
);
GO
...
--Вибір всіх даних з таблиці SALES
SELECT * FROM SALES;

--Видалення таблиці SALES
DROP TABLE SALES;

--Видалення бази даних SHOP
USE master;
DROP DATABASE SHOP;
